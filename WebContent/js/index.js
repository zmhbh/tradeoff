...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  function onRequestClick() {
    ...
    var companies = $('#querycompanies').val();
    var values = $('#queryvalues').val();


    $.ajax({
        url : "http://54.191.103.199:8080/tradeoffParser/webapi/parser/select",
        dataType:"JSONP",
        jsonpCallback: 'callback',
        data:{companies:companies,values:values},
        type: 'GET',
  ...
  function onOptionClick(){
      ...
      var val = $(this).val();
      var old = $('#queryvalues').val();

      $('#queryvalues').val(old+val+',');

  }

  function onRequestValueClick(){

    var companies = $('#querycompanies').val();

    $.ajax({
        url : "http://54.191.103.199:8080/tradeoffParser/webapi/parser/allkey",
        dataType:"JSONP",
        jsonpCallback: 'callback',
        ...
        success:function(data)
        {
          ...
          for(var i = 0; i < data.length; i++) {
            alert(data[i]);
            $('#valuelist').append($("<option>"+data[i]+"</option>").val(data[i])); 
          }
        }
    });
...
function onError(error) {
  ...
  // request for value button
  $('#requestvalue').click(onRequestValueClick);

  // request for files button
  $('#request').click(onRequestClick);

  // value choosen
  $('#valuelist').change(onOptionClick);
